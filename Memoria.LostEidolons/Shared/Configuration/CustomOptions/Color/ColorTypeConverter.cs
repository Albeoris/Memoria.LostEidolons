...
namespace Memoria.LostEidolons.Configuration.Color;

public static class ColorTypeConverter
{
    private static Type SupportedType { get; } = typeof(UnityEngine.Color);
    private static String NoneColorName { get; } = "None";

    private static readonly Dictionary<String, UnityEngine.Color> ColorsByName = new();
    private static readonly Dictionary<UnityEngine.Color, String> NamesByColor = new();
    ...
    private static String ConvertToString(Object boxed, Type type)
    {
        ...
        UnityEngine.Color color = (UnityEngine.Color)boxed;
        if (color == default)
    ...
    {
        PropertyInfo[] properties = typeof(UnityEngine.Color).GetProperties(BindingFlags.Static | BindingFlags.Public);
        foreach (PropertyInfo property in properties)
        {
            ...
            UnityEngine.Color color = (UnityEngine.Color)property.GetValue(null);
    ...
    public static String ToHex(UnityEngine.Color color)
    {
    ...
    public static UnityEngine.Color FromHex(String str)
    {
        // if (str.StartsWith("#"))
        //     str = str.Substring(1);
